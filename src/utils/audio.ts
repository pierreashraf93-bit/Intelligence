...
export const soundEngine = {
  ...
  // Interface button tactical click
  playUiClick(enabled: boolean = true) {
    if (!enabled) return;
    try {
      const ctx = getAudioContext();
      if (!ctx) return;
  ...
  // Dossier paper handling sound
  playDossierOpen(enabled: boolean = true) {
    if (!enabled) return;
    try {
      const ctx = getAudioContext();
      if (!ctx) return;
  ...
  playTelemetryBeep(enabled: boolean = true) {
    if (!enabled) return;
    try {
      const ctx = getAudioContext();
      if (!ctx) return;
